...
//		String hdfsFileChecksum = fileChecksum.get("bytes").toString();
  ...
  private boolean checkErrorChecksumDirExists(String filePath) throws IOException {
    Try<FileStatus> status = webHdfsFacade.getFileStatus(filePath);
    if (status.isFailure()) {
      if (status.failed().get() instanceof WebHdfsException) {
        if (((WebHdfsException) status.failed().get()).statusCode() == 404) return false;
      }
    }
    return true;
  }
  ...
  private boolean makeErrorChecksumDir(String dirPath) throws IOException {
    Try<Object> created = webHdfsFacade.mkdirs(dirPath);
    if (created.isSuccess() && created.get().toString().equalsIgnoreCase("true"))
      return true;
    else return false;
  }
   ...
   *
   * @param source errorChecksumFile path
   * @param dest   error checksum directory
   * @throws IOException
   */
  private void moveErrorChecksumFile(String source, String dest) throws IOException {
    webHdfsFacade.rename(source, dest);
  }
